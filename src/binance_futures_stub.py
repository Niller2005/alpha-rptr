...
class BinanceFuturesStub(BinanceFutures):
    ...
    def entry(self, id, long, qty, limit=0, stop=0, post_only=False, when=True):
        ...
        if limit > 0 or stop > 0:
            self.open_orders.append(
                {"id": id, "long": long, "qty": ord_qty, "limit": limit, "stop": stop, "post_only": post_only})
        else:
            self.commit(id, long, ord_qty, self.get_market_price(), True)
            return
    ...
    def commit(self, id, long, qty, price, need_commission=False):
        ...
        if (self.get_position_size() > 0 >= order_qty) or (self.get_position_size() < 0 < order_qty):
            if self.get_position_avg_price() > price:
                close_rate = ((self.get_position_avg_price() - price) /
                              price - commission) * self.get_leverage()
                profit = -1 * self.get_position_size() * close_rate
            else:
                close_rate = ((price - self.get_position_avg_price()) /
                              self.get_position_avg_price() - commission) * self.get_leverage()
        ...
        if next_qty != 0:
            ...
            if long and self.position_size < next_qty:
                self.position_avg_price = (
                    self.position_avg_price * self.position_size + price * qty) / next_qty
    ...
    def eval_exit(self):
        ...
        if self.get_position_avg_price() > price:
            close_rate = ((self.get_position_avg_price() - price) /
                          price - self.get_commission()) * self.get_leverage()
